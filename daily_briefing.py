...
def generate_briefing():
    ...
    自然帶到過去的做法與困境、現在的突破方式與影響、未來的發展方向與未解問題、
    以及一個值得思考的延伸問題，不要在段落內再拆出條列項目或粗體小標籤。

    寫作語氣可以偶爾夾雜外商公司常見的中英夾雜說法，但每一段最多只能出現 3-5 次英文術語，
    不要每句話都夾英文，其餘部分一律用流暢正常的中文書寫。
    每次夾英文時，講完英文要立刻用中文解釋一次，例如「這牽涉到 alignment，也就是對齊問題」這種寫法。

    全文請勿使用括號（包含中括號、全形括號）來補充翻譯或說明，一律用「也就是」「也叫做」
    這類口語銜接詞直接接起來，因為語音朗讀遇到括號會有不自然的停頓：

    以下 # 開頭那一行請直接寫成完整標題文字（不要用中括號當佔位符），格式為
    「AI 每日跨域破局分析：{today_str} 加上當日新聞的具體主題」：

    # AI 每日跨域破局分析：{today_str}
    ## 最新即時新聞
    （第一段：說明消息來源、發生的事、以及這件事代表的意義）
    （接下來直接繼續寫 3-5 段連貫的文章，不要加標題，依序自然帶到：
